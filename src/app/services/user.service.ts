    ...
    return new Promise((resolve, reject) => { //promise嵌套，注意调用次序
      this.rsa.Encrypt(pwd).then(res => {
        ...
        this.http.post(this.url.loginUrl, data).toPromise().then(res => {
          ...
          this.storage.set('username',JSON.stringify(res));
          this.storage.set('status',JSON.stringify(res['status']));
          this.storage.set('isLogin',true);
          this.storage.set('token',JSON.stringify(res['data'].token));
          this.storage.set('key',JSON.stringify(res['data'].user.key));
